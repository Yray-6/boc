...
export async function GET(request: Request, { params }: RouteParams) {
  ...
  if (!Number.isFinite(id)) {
    return NextResponse.json({ detail: "Invalid image id" }, { status: 400 });
  }

  try {
    const upstream = await adminGetPropertyImage(token, slug, id);
...
export async function DELETE(request: Request, { params }: RouteParams) {
  ...
  try {
    const upstream = await adminDeletePropertyImage(token, slug, id);
