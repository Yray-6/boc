...
interface PropertiesHeroProps {
  /** Shown as “N properties found” */
  foundCount: number;
}

/** Full-width band (~20vh) with headline; nav + logo overlay the image. */
export function PropertiesHero({ foundCount }: PropertiesHeroProps) {
  const foundLabel =
    foundCount === 1 ? "1 property found" : `${foundCount} properties found`;

  return (
    <div className="relative w-full">
      <div className="relative h-[30dvh] w-full max-w-none overflow-hidden text-white">
        <Image
        ...
        <div
          className="pointer-events-none absolute inset-y-0 left-0 w-[min(100%,720px)] bg-linear-to-r from-[#ececec] via-[#ececec]/65 to-transparent"
          aria-hidden
        />
        ...
        <div className="relative z-10 mx-auto h-full w-full max-w-[1440px] px-4 sm:px-6 lg:px-[85px]">
          ...
          <div className="absolute bottom-3 left-4 z-10 max-w-[min(100%,520px)] pr-2 sm:bottom-4 sm:left-6 lg:bottom-5 lg:left-[85px]">
            <h1 className="text-2xl font-medium leading-tight text-[#2a478d] sm:text-2xl md:text-3xl lg:text-4xl [font-family:var(--font-playfair)]">
              Properties for Sale
            </h1>
